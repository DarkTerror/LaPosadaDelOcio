...
void ChatLog::ChatMsg(Player *player, std::string &msg, uint32 type)
{
    ...
    // Quitar las comillas para lograr guardar estas frases en la bbdd, las reemplazaremos por "
    int i=0;
    std::string buscame="'";
    std::string reemplazame="\\\"";
    while(i==0)
    {
        std::string::size_type pos= que.find (buscame, 0);
        if (pos==std::string::npos)
            break;
        que.replace (pos, 1, reemplazame);
    }

    LogDatabase.PExecute("INSERT INTO lexxic(fecha,tipo,de,para,que) VALUES(NOW(), '%s', '%s', '%s', '%s')", tipo.c_str(), de.c_str(), para.c_str(), que.c_str());
...
void ChatLog::GuildMsg(Player *player, std::string &msg, bool officer)
{
    ...
    de.append("[");
    tipo.append((officer ? "{GUILD OFF}" : "{GUILD}"));

    if (!player->GetGuildId())
    {
        log_str.append("[unknown guild] ");
        para.append("[unknown guild]");
    }
    else
    {
...
void ChatLog::RaidMsg(Player *player, std::string &msg, uint32 type)
{
    ...
    switch (type)
    {
        ...
        tipo.append("{RAID LEADER}");
        break;

        case CHAT_MSG_RAID_WARNING:
        log_str.append("]->RAID_WARN:");
        tipo.append("{RAID WARN}");
        break;

        default:
        log_str.append("]->RAID_UNKNOWN:");
        tipo.append("{RAID UNKNOWN}");
    }
...
void ChatLog::BattleGroundMsg(Player *player, std::string &msg, uint32 type)
{
    ...
    switch (type)
    {
        ...
        tipo.append("{BG LEADER}");
        break;

        default:
        log_str.append("]->BG_UNKNOWN:");
        tipo.append("{BG UNKNOWN}");
    }
