...
void Aura::HandleShapeshiftBoosts(bool apply)
{
    ...
    if(apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (form == FORM_BEAR || form == FORM_DIREBEAR)
            {
                Unit::AuraList const& modAuras = GetTarget()->GetAurasByType(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE);
                for (Unit::AuraList::const_iterator i = modAuras.begin(); i != modAuras.end(); ++i)
                {
                    if ((*i)->GetSpellProto()->SpellFamilyName==SPELLFAMILY_DRUID &&
                        (*i)->GetSpellProto()->SpellIconID == 961)
                    {
                        int32 bp = (*i)->GetSpellProto()->CalculateSimpleValue(EFFECT_INDEX_2);
                        if (bp)
                            GetTarget()->CastCustomSpell(GetTarget(), 62069, &bp, NULL, NULL, true, NULL, this);
                        break;
                    }
                }
            }
...
{
    if (!Real || !Apply || !GetTarget() || GetTarget()->GetTypeId() != TYPEID_UNIT)
        return;
    Unit* caster = GetCaster();
    Unit* creator = ObjectAccessor::GetUnit(*GetTarget(),GetTarget()->GetCreatorGuid());
    Creature* pImmage = (Creature*)GetTarget();
