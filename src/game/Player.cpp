    ...
    if (!(currFields & val))
    {
        ...
        else if(p->area_level > 0)
        {
            ...
            else
            {
                ...
                XP = uint32(sObjectMgr.GetBaseXP(p->area_level)*sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE_PREMIUM));
    ...
    if(GetSession()->GetSecurity() < SEC_GAMEMASTER)
    {
        ...
        for(std::vector<uint32>::iterator itr = prof_skills.begin(); itr != prof_skills.end(); ++itr)
        {
            ...
            if(HasSkill(skill_id))
            {
                ++prof_count;
                if(prof_count > 2)
                    sLog.outError("Player %s has more than two professions!",GetName());
                    LogDatabase.PExecute("INSERT INTO `player_profesion_max` (`player`,`num_profesiones`) VALUES ('%s','%u')",GetName(),prof_count);
            }
        }
    }
